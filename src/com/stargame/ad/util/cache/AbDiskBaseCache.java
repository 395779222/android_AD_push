...
/**
 * 
 * 名称：AbDiskBasedCache.java 描述：磁盘缓存
 * 
 * @author hxy
 * @version v1.0
 * @date：2015-6-11
...
public class AbDiskBaseCache implements AbDiskCache {
	...
	/**
	 * 默认缓存空间的构造. the default maximum cache size of 5MB.
	 * 
	 * @param rootDirectory
	 *            The root directory of the cache.
	 */
	public AbDiskBaseCache(File rootDirectory) {
		this(rootDirectory, AppConfig.MAX_DISK_USAGE_INBYTES);
	}
	...
	public synchronized void initialize() {
		if (!mRootDirectory.exists()) {
			if (!mRootDirectory.mkdirs()) {
				LogUtil.e(AbDiskBaseCache.class,
						"缓存目录创建失败，" + mRootDirectory.getAbsolutePath());
			}
			return;
		}
	...
	public synchronized Entry get(String key) {
		...
		try {
			...
			byte[] data = AbStreamUtil.stream2Bytes(cis,
	...
	public synchronized void put(String key, Entry entry) {
		...
		if (!deleted) {
			LogUtil.d(AbDiskBaseCache.class,
					"缓存文件删除失败" + file.getAbsolutePath());
		}
	}
	...
	private void pruneIfNeeded(int neededSpace) {
		...
		if (LogUtil.D) {
			LogUtil.d(AbDiskBaseCache.class,
					"pruned %d files, %d bytes, %d ms", prunedFiles,
