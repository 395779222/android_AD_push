...
import com.stargame.ad.util.AbFileUtil;
import com.stargame.ad.util.AppUtil;
import com.stargame.ad.util.LogUtil;
import com.stargame.ad.util.global.AppConfig;
import com.stargame.ad.util.global.AppException;
import com.stargame.ad.util.http.entity.MultipartEntity;
import com.stargame.ad.util.http.ssl.EasySSLProtocolSocketFactory;
import com.stargame.ad.util.task.thread.AbThreadFactory;
...
/**
 * 名称：AbHttpClient.java 描述：Http客户端
 * 
 * @author JXM
 * @version v1.0
 * @date：2015-06-11
		...
		try {
			...
			if (!AppUtil.isNetworkAvailable(mContext)) {
				...
				responseListener.sendFailureMessage(
						...
						AppConfig.CONNECT_EXCEPTION, new AppException(
								AppConfig.CONNECT_EXCEPTION));
				return;
			}
			...
			String response = httpClient.execute(httpGet,
					...
					mHttpContext);
			LogUtil.d(mContext, "[HTTP GET]:" + url + ",result：" + response);
		} catch (Exception e) {
			e.printStackTrace();
			// 发送失败消息
			responseListener.sendFailureMessage(AbHttpStatus.UNTREATED_CODE,
					e.getMessage(), new AppException(e));
		} finally {
	...
	private void doPost(String url, AbRequestParams params,
		...
		try {
			...
			if (isContainFile) {
				...
				LogUtil.d(mContext, "[HTTP POST]:" + url + ",包含文件域!");
			}
			// 取得HttpResponse
			response = httpClient.execute(httpPost,
					new RedirectionResponseHandler(url, responseListener),
					mHttpContext);
			LogUtil.d(mContext, "request：" + url + ",result：" + response);
	...
	public void writeResponseData(Context context, HttpEntity entity,
		...
		try {
			...
			responseListener.sendFailureMessage(
					...
					AppConfig.SOCKET_TIMEOUT_EXCEPTION, e);
		} finally {
			try {
				if (inStream != null) {
					inStream.close();
		...
		public void handleMessage(Message msg) {
				...
				if (response != null) {
					if (responseListener instanceof AbStringHttpResponseListener) {
						if (response.length >= 2) {
							...
							LogUtil.e(mContext, "SUCCESS_MESSAGE "
									+ AppConfig.MISSING_PARAMETERS);
						}
			...
			case FAILURE_MESSAGE:
				...
				if (response != null && response.length >= 3) {
					...
					LogUtil.e(mContext, "FAILURE_MESSAGE "
							+ AppConfig.MISSING_PARAMETERS);
				}
				break;
			case START_MESSAGE:
			...
			case PROGRESS_MESSAGE:
				...
				if (response != null && response.length >= 2) {
					...
					LogUtil.e(mContext, "PROGRESS_MESSAGE "
							+ AppConfig.MISSING_PARAMETERS);
				}
				break;
			case RETRY_MESSAGE:
		...
		public String handleResponse(HttpResponse response)
			...
			// 301 302进行重定向请求
				...
				mResponseListener.sendFailureMessage(statusCode,
						AppConfig.NOT_FOUND_EXCEPTION, new AppException(
								AppConfig.NOT_FOUND_EXCEPTION));
			} else if (statusCode == HttpStatus.SC_FORBIDDEN) {
				// 403
				mResponseListener.sendFailureMessage(statusCode,
						AppConfig.FORBIDDEN_EXCEPTION, new AppException(
								AppConfig.FORBIDDEN_EXCEPTION));
			} else {
				mResponseListener.sendFailureMessage(statusCode,
						AppConfig.REMOTE_SERVICE_EXCEPTION, new AppException(
								AppConfig.REMOTE_SERVICE_EXCEPTION));
			}
			return null;
		}
	}
