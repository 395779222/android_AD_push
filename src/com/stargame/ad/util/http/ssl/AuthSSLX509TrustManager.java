package com.stargame.ad.util.http.ssl;
...
public class AuthSSLX509TrustManager implements X509TrustManager {
    ...
    public void checkClientTrusted(X509Certificate[] certificates, String authType)
        ...
        if (certificates != null) {
            for (int c = 0; c < certificates.length; c++) {
                ...
                LogUtil.d(TAG,"  Client certificate " + (c + 1) + ":");
                LogUtil.d(TAG,"  Subject DN: " + cert.getSubjectDN());
                LogUtil.d(TAG,"  Signature Algorithm: " + cert.getSigAlgName());
                LogUtil.d(TAG,"  Valid from: " + cert.getNotBefore());
                LogUtil.d(TAG,"  Valid until: " + cert.getNotAfter());
                LogUtil.d(TAG,"  Issuer: " + cert.getIssuerDN());
            }
        }
    ...
    public void checkServerTrusted(X509Certificate[] certificates, String authType)
        ...
        if (certificates != null) {
            for (int c = 0; c < certificates.length; c++) {
                X509Certificate cert = certificates[c];
                LogUtil.d(TAG,"  Server certificate " + (c + 1) + ":");
                LogUtil.d(TAG,"  Subject DN: " + cert.getSubjectDN());
                LogUtil.d(TAG,"  Signature Algorithm: " + cert.getSigAlgName());
                LogUtil.d(TAG,"  Valid from: " + cert.getNotBefore());
                LogUtil.d(TAG,"  Valid until: " + cert.getNotAfter());
