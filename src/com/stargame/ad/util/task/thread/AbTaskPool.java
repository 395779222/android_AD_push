 ...
 */
package com.stargame.ad.util.task.thread;
...
import com.stargame.ad.util.task.AbTaskListListener;
import com.stargame.ad.util.task.AbTaskObjectListener;
/**
 * 名称：AbTaskPool.java 描述：用andbase线程池
 * 
 * @author hxy
 * @version v1.0
 * @date：2015-06-11
	...
	private static Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (item.getListener() instanceof AbTaskListListener) {
				((AbTaskListListener) item.getListener())
						.update((List<?>) result.get(item.toString()));
				...
				try {
					...
					if (item.getListener() != null) {
						if (item.getListener() instanceof AbTaskListListener) {
							...
							result.put(item.toString(),
									((AbTaskObjectListener) item.getListener())
											.getObject());
						} else {
