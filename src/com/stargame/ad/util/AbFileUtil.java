...
public class AbFileUtil {
	 ...
	 * @return Bitmap 新图片
	 */
	public static Bitmap getBitmapFromURL(String url, int desiredWidth,
			int desiredHeight) {
		Bitmap bitmap = null;
		try {
			bitmap = AbImageUtil.getBitmap(url, desiredWidth, desiredHeight);
		} catch (Exception e) {
			LogUtil.d(AbFileUtil.class, "下载图片异常：" + e.getMessage());
		}
		return bitmap;
	}
	...
	public static Bitmap getBitmapFromSrc(String src) {
		...
		try {
			...
			LogUtil.d(AbFileUtil.class, "获取图片异常：" + e.getMessage());
		}
		return bit;
	}
	...
	public static String downloadFile(String url, String dirPath) {
		...
		try {
			...
			e.printStackTrace();
			LogUtil.e(AbFileUtil.class, "有文件下载出错了,已删除");
	...
	public static int getContentLengthFromUrl(String Url) {
		...
		try {
			...
			e.printStackTrace();
			LogUtil.d(AbFileUtil.class, "获取长度异常：" + e.getMessage());
		}
	...
	public static String getRealFileNameFromUrl(String url) {
		...
		try {
			...
			e.printStackTrace();
			LogUtil.e(AbFileUtil.class, "网络上获取文件名失败");
		}
		return name;
	}
	 ...
	 *            完整sd卡路径
	 */
	public static void copyAssets2SD(Context context, String assetDir,
			String outDir) {
		String[] files;
		try {
